...
static Node* get_id_op(Token *const tokens, Id *const ids, size_t *const pointer, ErrList *const list);
...
Node* get_operation(Token *const tokens, Id *const ids, size_t *const pointer, ErrList *const list)
{
    ...
    while (!is_end || *pointer >= TOKEN_AMT)
    {
        ...
        {
            if (tokens[*pointer].value == BR || tokens[*pointer].value == CALC_BR)
            {
                (*pointer)++;
                ...
                if (tokens[*pointer].type != OP && (tokens[*pointer].value != BR || tokens[*pointer].value == CALC_BR))
                {
        ...
        else if (tokens[*pointer].type == ID)
        {
            ...
            {
                printf("ID OP\n");
                val = get_id_op(tokens, ids, pointer, list);
            }
...
Node* get_id_op(Token *const tokens, Id *const ids, size_t *const pointer, ErrList *const list)
{
    REC_ASSERT
    ...
    {
        //определение функции
        *pointer += 2;  //проскочили и текущее значение, и скобку

        Node* operations = get_operation(tokens, ids, pointer, list);
        RETURN_PTR
        ...
        while (!is_end)
        {
            ...
            if (tokens[*pointer].type == OP && tokens[*pointer].value == BR)
            {
                (*pointer)++;  //чтобы пропустить эту скобку
                is_end = true;
            }
        }

        val = make_node(ID, tokens[*pointer].value, operations, nullptr, list);
    }
    else 
    {
        printf("USE ID OP\n");
        val = make_node(ID, tokens[*pointer].value, nullptr, nullptr, list);
        (*pointer)++;
    }
    RETURN_PTR
...
Node* get_if(Token *const tokens, Id *const ids, size_t *const pointer, ErrList *const list)
{
    ...
    if (tokens[*pointer].type == OP && tokens[*pointer].value == BR)
    {
        ...
        right = get_operation(tokens, ids, pointer, list);
        RETURN_PTR

        (*pointer)++;  //пропустил точку перед скобкой
        ...
        while (!is_end)
        {
            ...
            //{
                ...
                right = make_node(OP, SEP, right, val2, list);
                RETURN_PTR
                (*pointer)++;
            //}
